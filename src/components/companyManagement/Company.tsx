...
// import { useGetAllCompanyQuery } from "@/redux/api/super-admin/companyManagement/superAdminCompanyManagementSliceApi";
...
//     {
//       id: 1,
//       companyName: "Dianne Russell Ltd",
...
//     {
//       id: 3,
//       companyName: "Darrell Steward LLC",
...
//     {
//       id: 6,
//       companyName: "Kristin Watson Solutions",
...
//             Company and monitor all users across your platform
//           </p>
//         </div>
...
//               All Company ({filteredUsers.length})
//             </h2>
...
} from "@/redux/api/super-admin/companyManagement/superAdminCompanyManagementSliceApi";
import DeleteConfirmModal from "@/share/DeteleConfirm/DeleteConfirm";
import { UserDetails } from "../userManagement/userDetails/Details";
...
const CompanyManagement: React.FC = () => {
  ...
  const [statsUpdate, { isLoading: statsLoading }] = useCompanyStatusMutation();
    ...
    data?.data?.map((company: any) => {
      ...
      return {
        ...
        subscription: activeSub
          ? activeSub.planSnapshot?.name === "ADVANCED"
            ? "Pro"
            : activeSub.planSnapshot?.name === "EXPERT"
              ? "Enterprise"
              : "Free"
          : "Free",
        createdDate: new Date(company.createdAt).toLocaleDateString(),
        users: company._count?.companyMembers ?? 0,
            ...
            <tbody className="divide-y divide-gray-200 bg-white">
              {filteredUsers.map((user) => (
                ...
                >
                  ...
                  <td className="whitespace-nowrap px-6 py-4">
                    <span
                      className={`rounded-md px-3 py-2 text-xs font-semibold ${getSubscriptionColor(
                        user.subscription,
                      )}`}
                    >
                      {user.subscription}
                    ...
                    <div className="relative">
                      ...
                      {activeActionMenu === user.id && (
                        <div className="absolute right-0 z-10 mt-2 w-48 rounded-lg border border-gray-200 bg-white shadow-lg">
                          <div className="py-2">
                            <Link
                              href={`/super-admin/company-management/company-details?id=${user.id}`}
                              className="flex w-full items-center gap-2 px-4 py-2 text-left transition-colors hover:bg-gray-50"
                            >
                              <User className="h-4 w-4" />
                              <span>View Details</span>
